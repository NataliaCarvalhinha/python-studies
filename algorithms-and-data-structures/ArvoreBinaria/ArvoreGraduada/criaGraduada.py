...
def criaArvore(simetrica, preOrdem):
    ...
    arvore = ArvoreBinariaBusca(popPre)
    
    noEsqSim , noDirSim = simetrica.split(arvore.valor)
    noEsqPre = preOrdem[:len(noEsqSim)]
    noDirPre = preOrdem[len(noEsqSim):]
    arvore.esq = criaArvore(noEsqSim, noEsqPre)
    arvore.dir = criaArvore(noDirSim, noDirPre)
    
    return arvore
    
simetrica = "LGRMSDNHTOUBEAIFPJVQXC"
preOrdem = "ABDGLMRSHNOTUECFIJPQVX"

print(criaArvore(simetrica, preOrdem))